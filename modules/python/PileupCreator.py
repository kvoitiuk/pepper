...
class PileupCreator:
    ...
    def chunk_images(summary, chunk_size, chunk_overlap):
        ...
        while True:
            image_chunk = image[:, chunk_start:chunk_end, :]
            pos_chunk = summary.genomic_pos[chunk_start:chunk_end]
            label_chunk = [0] * (chunk_end - chunk_start)

            assert (image_chunk.shape[1] == len(pos_chunk) == len(label_chunk))

            padding_required = chunk_size - image_chunk.shape[1]

            if padding_required > 0:
                label_chunk = label_chunk + [0] * padding_required
                pos_chunk = pos_chunk + [(-1, -1)] * padding_required
                empty_chunk = np.array([[[0, 0, 0]] * padding_required] * ImageSizeOptions.IMAGE_HEIGHT)
                image_chunk = np.append(image_chunk, empty_chunk, 1)

            assert (image_chunk.shape[1] == len(pos_chunk) == len(label_chunk) == ImageSizeOptions.SEQ_LENGTH)
    ...
    def chunk_images_train(summary, chunk_size, chunk_overlap):
        ...
        for i in range(len(bad_indices)):
            ...
            while True:
                ...
                assert (image_chunk.shape[1] == len(pos_chunk) == len(label_chunk) == chunk_size)
    ...
    def create_pileup(self, truth_bam_handler, train_mode, realignment_flag):
        ...
        if train_mode:
            ...
            truth_reads, insert_map = truth_bam_handler.get_reads(self.chromosome_name,
            ...
            for region in truth_regions:
                ...
                all_reads, insert_map = self.bam_handler.get_reads(self.chromosome_name,
                                                       read_start,
                                                       read_end,
                                                       include_supplementary,
                                                       0,
                ...
                summary_generator = HELEN.PileupGenerator(ref_seq,
                ...
                summary_generator.generate_labels(truth_read, region_start, region_end)
                # summary_generator.debug_print()
            ...
            pileup_generator = HELEN.PileupGenerator(ref_seq,
                                                     self.chromosome_name,
                                                     self.region_start_position,
                                                     self.region_end_position,
                                                     False)
            pileup_generator.generate_summary(all_reads,
                                              self.region_start_position,
                                              self.region_end_position)
            # pileup_generator.debug_print()

            images, labels, positions, chunk_ids = \
                self.chunk_images(pileup_generator,
                                  chunk_size=ImageSizeOptions.SEQ_LENGTH,
                                  chunk_overlap=ImageSizeOptions.SEQ_OVERLAP)
